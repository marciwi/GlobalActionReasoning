...
for a=1:nrA
  ...
  %else
    %observation=problem.observation{a}; %doesn't work...
	%observation=problem.observation(:,a,:);%test?
	%observation=problem.observation;%test2?
	%already done in line 27...
  end
  for o=1:nrO
    if problem.useSparse
      ...
      for k=1:nrInV
        for s1=1:nrS
		  ...
		  GammaAOS=GammaAO; %test?..added to circumvent errors!
        end
      end
    end
  end
